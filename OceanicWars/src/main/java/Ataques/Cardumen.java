...
public class Cardumen extends Ataque {
    ...
    public Cardumen(Hero hero, Jugador contrincante) {
        ...
        this.golpePez = (int) (30 * (1 + hero.getFuerzaAtaque() / 100.0)); // Se hace daño basado en la fuerza del heroe
    }
        ...
        for (int i = 0; i < cantidadPeces; i++) { // Para cada pez
            ...
            casilla.getBitacora()
                    .add("La casilla (" + casilla.getX() + ", " + casilla.getY()
                            + ") fue golpeada por un pez del 'Cardumen' de " + hero.getNombre() + ", recibiendo "
                            + this.golpePez + " puntos de daño"); // Mensaje agregado a la bitácora de la casilla
        }
    }
