...
public class EelAttack extends Ataque {
    ...
    public EelAttack(Hero hero, Jugador contrincante) {
        ...
        this.golpeDescarga = (int) (10 * (1 + hero.getFuerzaAtaque() / 100.0)); // Se hace daño basado en la fuerza del
                                                                                // heroe
    }
    ...
    public void ejecutar() {
        for (int i = 0; i < this.cantidadAnguilas; i++) {
            ...
            casilla.getBitacora().add("La casilla (" + casilla.getX() + ", " + casilla.getY()
                    + ") fue seleccionada por una anguila del 'Eel Attack' de " + hero.getNombre());
            ...
            for (int j = 0; j < this.cantidadDescargas; j++) { // For para dar las descargas
                ...
                casilla.getBitacora()
                        .add("La casilla (" + casilla.getX() + ", " + casilla.getY()
                                + ") fue golpeada por una descarga de anguila del 'Eel Attack' de " + hero.getNombre()
                                + ", recibiendo " + this.golpeDescarga + " puntos de daño"); // Mensaje agregado a la
