...
public class PoseidonThunders extends Ataque {
    // Constructor
    public PoseidonThunders(Hero hero, Matriz matriz) {
        super(hero);
    }
    ...
    // Cantidad de rayos generada
        ...
        for (int i = 0; i < cantidadRayos; i++) {
            ...
            casilla.getBitacora().add("La casilla (" + casilla.getX() + ", " + casilla.getY() + ") fue seleccionada como epicentro de una onda del 'Poseidon Thunders' de " + hero.getNombre() + ", recibiendo " + casilla.getVida() + " puntos de daño");  // Mensaje agregado a la bitácora de la casilla
            ...
            for (Casilla c : matriz.getCasillasActivas()) {  // Revisamos en las casillas activas
                if (matriz.IsCasillaEnRadio(casilla.getX(), casilla.getY(), c.getX(), c.getY(), radioObtenido)) {
                    ...
                    c.getBitacora().add("La casilla (" + c.getX() + ", " + c.getY() + ") fue golpeada por una onda expansiva del 'Poseidon Thunders' de " + hero.getNombre() + ", recibiendo " + c.getVida() + " puntos de daño");  // Mensaje agregado a la bitácora de la casilla
                }
            }
        }
    }
