    ...
    public void ejecutar() {
        for (int i = 0; i < this.cantidadPulpos; i++) {
            for (int j = 0; j < 8; j++) { // Va desde 0 a 7 para representar los 8 tentáculos
                // Elección de casilla
                int golpeTentaculo = (int) (25 * (1 + hero.getFuerzaAtaque() / 100.0)); // Se hace daño basado en la
                                                                                        // fuerza del heroe
                ...
                casilla.getBitacora()
                        .add("La casilla (" + casilla.getX() + ", " + casilla.getY()
                                + ") fue golpeada por un tentáculo del 'Pulp' de " + hero.getNombre() + ", recibiendo "
                                + golpeTentaculo + " puntos de daño"); // Mensaje agregado a la bitácora de la casilla
            }
        }
    }
