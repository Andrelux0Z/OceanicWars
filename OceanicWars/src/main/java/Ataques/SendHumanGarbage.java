...
public class SendHumanGarbage extends Ataque {
    ...
    public void ejecutar() {
        ...
        try {
            ...
            int golpeTonelada = (25 * (1 + hero.getFuerzaAtaque()));  // La basura pega más si el ataque está potenciado (fuera del cíclo para que se aplique a todos los pedazos)
            for (int i = 0; i < toneladasObtenidas; i++) {  // Se genera la cantidad de basura necesaria
                ...
                c.recibirGolpe(golpeTonelada);  // Golpea a la casilla
                c.getBitacora().add("La casilla (" + c.getX() + ", " + c.getY() + ") fue golpeada por un pedazo de basura del 'Send Human Garbage' de " + hero.getNombre() + ", recibiendo " + golpeTonelada + " puntos de daño");  // Mensaje agregado a la bitácora de la casilla
