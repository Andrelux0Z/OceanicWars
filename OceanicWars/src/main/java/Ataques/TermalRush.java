...
public class TermalRush extends Ataque {
    ...
    public void ejecutar() { 
        ...
        try {
            ...
            casilla.getBitacora().add("La casilla (" + casilla.getX() + ", " + casilla.getY() + ") fue seleccionada como el épicentro del 'Termal Rush' de " + hero.getNombre());
            this.radioObtenido += ((Volcan)casilla.getObjetoPresente()).getRadio();  // Se obtiene el radio del volcán, el cual es sumado al radio adicional que habiamos sacado en un inicio
            int golpeCalentamiento = ((Volcan)casilla.getObjetoPresente()).getRadio();  // El daño progresivo es hecho también en función del tamaño del volcán
            
            
            for (Casilla c : matriz.getCasillasActivas()) {
                if (matriz.IsCasillaEnRadio(casilla.getX(), casilla.getY(), c.getX(), c.getY(), this.radioObtenido)) {  // Si la casilla está dentro del rango
                    c.recibirGolpe(golpeCalentamiento * this.tiempoActiva);
                    c.getBitacora().add("La casilla (" + casilla.getX() + ", " + casilla.getY() + ") fue golpeada por la creación del volcán del 'Volcano Raising' de " + hero.getNombre() + ", recibiendo " + golpeCalentamiento * this.tiempoActiva + " puntos de daño");
                }
            }
        }
