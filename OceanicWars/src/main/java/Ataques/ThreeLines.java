...
public class ThreeLines extends Ataque {
    ...
    public ThreeLines(Hero hero, Jugador contrincante, ArrayList<Point> casillasElegidas) {
        ...
        this.alcanceObtenido = (int) ((1 + rand.nextInt(4)) * (1 + hero.getFuerzaAtaque() / 100.0)); // Se tiene un
                                                                                                     // alcance de
                                                                                                     // ataque en
                                                                                                     // función de la
                                                                                                     // fuerza del héroe
                ...
                switch (direccion) {
                    case ARRIBA: // Al elegir la dirección ARRIBA
                        for (int i = 0; i < this.alcanceObtenido; i++) {
                            if ((casilla.getY() - i) < 0) { // Verifica para no salirse de los límites: haciendo un
                    ...
                    case ABAJO: // Al elegir la dirección ABAJO
                        for (int i = 0; i < this.alcanceObtenido; i++) {
                            if ((casilla.getY() + i) > this.matriz.getCantidadFilas() - 1) { // Verifica para no salirse
                    ...
                    case IZQUIERDA: // Al elegir la dirección IZQUIERDA
                        for (int i = 0; i < this.alcanceObtenido; i++) {
                            if ((casilla.getX() - i) < 0) { // Verifica para no salirse de los límites: haciendo un
                                                            // break si llega a un borde
                                break;
                            }
                    ...
                    case DERECHA: // Al elegir la dirección DERECHA
                        for (int i = 0; i < this.alcanceObtenido; i++) {
                            if ((casilla.getX() + i) > this.matriz.getCantidadColumnas() - 1) { // Verifica para no
                                                                                                // salirse de los
                                                                                                // límites: haciendo un
                                                                                                // break si llega a un
                                                                                                // borde
