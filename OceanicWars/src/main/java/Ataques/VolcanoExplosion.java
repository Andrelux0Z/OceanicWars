...
public class VolcanoExplosion extends Ataque {
    ...
    public VolcanoExplosion(Hero hero, Jugador contrincante, Point casillaElegida) {
        ...
        this.golpePiedra = (int) (20 * (1 + hero.getFuerzaAtaque() / 100.0)); // Golpe aplicado por cada piedra de la
                                                                              // erupción
    }
    ...
    public void ejecutar() {
        ...
        try {
            ...
            casilla.getBitacora().add("La casilla (" + casilla.getX() + ", " + casilla.getY()
                    + ") fue seleccionada como el épicentro del 'Volcano Explosion' de " + hero.getNombre());
            int piedrasObtenidas = ((Volcan) casilla.getObjetoPresente()).getRadio() * 10; // Se obtiene el radio
                                                                                           // original del volcán y se
                                                                                           // multiplica por 10
            for (int i = 0; i < piedrasObtenidas; i++) {
                ...
                c.getBitacora()
                        .add("La casilla (" + c.getX() + ", " + c.getY()
                                + ") fue golpeada por una piedra del 'Volcano Explosion' de " + hero.getNombre()
                                + ", recibiendo " + this.golpePiedra + " puntos de daño"); // Mensaje agregado a la
